...
QT_FOLDER="qt-everywhere-opensource-src-$QT_VERSION"
...
mkdir qt-build
cd qt-build
../$QT_FOLDER/configure QMAKE_APPLE_DEVICE_ARCHS="x86_64 arm64" -opensource -confirm-license -nomake examples -nomake tests -no-openssl -securetransport -prefix "$QT_PREFIX"
make -j4
make -j4 install
